...
- **No worker publication (structural, not a shell sandbox).** Publication authority is separated at the runner layer: the worker is never granted a publish tool, runs only inside a disposable detached worktree that the runner removes, and the runner — not the worker — owns candidate validation and the separate `global-build publish` command. The worker's own prompt contract forbids any push/publish. The canonical worker permission block *also* lists `git push` and every remote/topology/ref command as `deny`; treat that list as a declared guardrail and defense-in-depth, not a hard runtime sandbox. In OpenCode 1.18.x non-interactive `run`, the session is auto-approved, so agent allow/deny *command-pattern* rules are not re-checked at the tool layer (only OpenCode's own small internal banned-command list — `curl`, `wget`, `nc`, `telnet`, browsers, … — is always enforced). The worker's inability to publish therefore rests on the runner architecture and the prompt contract, not on command-pattern matching.
- **General local shell capability; Git authority as guardrails.** Ordinary local shell execution is BUILD *capability*, not authority, so the worker's `bash` permission is broadly `allow` (covering `go test`/`go vet`, `npm`/`pnpm`/`yarn`, `pytest`, `cargo`, `make`, repo-local scripts, codegen/proof, etc.) rather than default-denied. On top of that broad allow, the canonical worker lists the dangerous Git operations as `deny` (`git merge*`, `git rebase*`, `git cherry-pick*`, `git fetch*`, `git pull*`, `git push*`, `git branch*`, `git tag*`, `git update-ref*`, `git remote*`, `git clone*`, `git reset*`, `git stash*`, `git worktree*`, `git clean*`, `git gc*`, `git prune*`, `git reflog*`, `git filter-branch*`, `git replace*`, `sudo*`). These are last-match-wins guardrails and declared intent: with broad shell access a determined process could invoke Git by another path (e.g. an absolute `/usr/bin/git` or a shell wrapper), and in OpenCode 1.18.x non-interactive `run` the session is auto-approved so the patterns are not re-enforced at the tool layer. The hard containment of Git publication/topology is the runner architecture above, not these patterns.
...
- **Deterministic acceptance invariant.** Exactly one non-merge commit on `admitted_base..candidate`; runner-owned ancestry/merge/clean checks; candidate ref created by the runner; OpenCode version exactly 1.18.x; worker response mechanically parsed against the strict result protocol.
- **Execution default.** Noninteractive prepared-task runner; one attempt; `steps=50`; investigator budget `3`; publication is a separate command and never part of a BUILD.
